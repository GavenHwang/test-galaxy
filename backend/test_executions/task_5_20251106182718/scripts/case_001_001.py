        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                try:
                    ...
                    logger.log_step(1, "navigate", "导航到登录页", "通过", step_duration, step_start_time=step_start)
                    if DEBUG:
                        print(f"  ✓ 步骤 1: 导航到登录页")
                except Exception as e:
                    step_duration = int((datetime.now() - step_start).total_seconds() * 1000)
                    ...
                    logger.log_step(1, "navigate", "导航到登录页", "失败", step_duration, str(e), screenshot_path, step_start)
                    raise
                ...
                try:
                    ...
                    step_duration = int((datetime.now() - step_start).total_seconds() * 1000)
                    logger.log_step(2, "type", "输入用户名", "通过", step_duration, step_start_time=step_start)
                    if DEBUG:
                        print(f"  ✓ 步骤 2: 输入用户名")
                except Exception as e:
                    step_duration = int((datetime.now() - step_start).total_seconds() * 1000)
                    ...
                    logger.log_step(2, "type", "输入用户名", "失败", step_duration, str(e), screenshot_path, step_start)
                    raise
                ...
                try:
                    ...
                    step_duration = int((datetime.now() - step_start).total_seconds() * 1000)
                    logger.log_step(3, "type", "输入密码", "通过", step_duration, step_start_time=step_start)
                    if DEBUG:
                        print(f"  ✓ 步骤 3: 输入密码")
                except Exception as e:
                    step_duration = int((datetime.now() - step_start).total_seconds() * 1000)
                    ...
                    logger.log_step(3, "type", "输入密码", "失败", step_duration, str(e), screenshot_path, step_start)
                    raise
                ...
                try:
                    ...
                    step_duration = int((datetime.now() - step_start).total_seconds() * 1000)
                    logger.log_step(4, "click", "点击登录", "通过", step_duration, step_start_time=step_start)
                    if DEBUG:
                        print(f"  ✓ 步骤 4: 点击登录")
                except Exception as e:
                    step_duration = int((datetime.now() - step_start).total_seconds() * 1000)
                    ...
                    logger.log_step(4, "click", "点击登录", "失败", step_duration, str(e), screenshot_path, step_start)
                    raise
